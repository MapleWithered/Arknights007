...
######## 修改的部分，加上下面还有很多获取path的地方都要用这个函数

Price = dict()
with open(os.path.join(get_path(), 'price.txt'), 'r', encoding='utf8') as f:
...
class MaterialPlanning(object):
    def __init__(self,
                 ...
                 url_rules='formula',
                 path_stats=os.path.join(get_path(), 'data', 'matrix.json'),
                 path_rules=os.path.join(get_path(), 'data', 'formula.json'),
                 update=False,
                 banned_stages={},
                 #                 expValue=30,
                 #                 ConvertionDR=0.18,
    ...
    def update(self,
        ...
        if time.time() - _last_update > 3600:
            force = True
