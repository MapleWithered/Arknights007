...
from . import ship
from .adb import ADB
from .imgreco import imgops, template
...
def gen_rect_matrix(img, bigger_rect=1) -> list[SkillRect]:
    anchor_delta = res.navigator.get_pos("/ship/change_scene/skill_matrix/anchor_delta")

    bumper_start_x = res.navigator.get_pos("/ship/change_scene/skill_matrix/bumper_start_x")
    bumper_y = res.navigator.get_pos("/ship/change_scene/skill_matrix/bumper_y")
    ...
    dx_small = res.navigator.get_pos("/ship/change_scene/skill_matrix/dx_small")
    dx = res.navigator.get_pos("/ship/change_scene/skill_matrix/dx")
    dy = res.navigator.get_pos("/ship/change_scene/skill_matrix/dy")
    size = res.navigator.get_pos("/ship/change_scene/skill_matrix/size")
    rect_list = []
    people_id = 0
...
def read_all_skill_icon_resource():
    ...
    for path, dir_list, file_list in os.walk(icon_path):
        for file_name in file_list:
            img_icon = cv2.imread(os.path.join(icon_path, file_name), cv2.IMREAD_UNCHANGED)
            width = int(img_icon.shape[0] / 720 * ADB.get_resolution().height)
...
def reco_matrix(img_gray, skillrect_list, skill_list, score_above_than=0.8, debug_show=False):
    reco_result = []
...
def choose_skill_generic(scene_skill_keyword, category_limit, self_conflict_skill, max_people, num_chosen):
    priority_dict: dict[str, int] = res.yaml.load_yaml("ship_skill/priority.yaml")['priority']
    category_dict = res.yaml.load_yaml("ship_skill/manufactory_category.yaml")[
...
def reco_skills(debug_show=False):
    ...
    reco_result = reco_matrix(img_gray, skill_rect_list, skill_mat, 0.2, debug_show=debug_show)
    if debug_show:
