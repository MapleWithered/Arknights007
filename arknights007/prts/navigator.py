...
def is_main_menu():
    ...
    pos_rect = res_nav.get_pos("/main_menu/gear")
...
def is_terminal():
    ...
    img_gray = ADB.screencap_mat(gray=True, std_size=True)
    pos_rect = res_nav.get_pos("/terminal/terminal_template")
    cropped = imgops.mat_crop(img_gray, Rect(*pos_rect))
    cropped = imgops.mat_pick_grey(cropped, 235, 20)
    if not os.path.exists(res_img.get_img_path(template_shortpath)):
        cv2.imwrite(res_img.get_img_path(template_shortpath), cropped)
    terminal_template = res_img.get_img_gray(template_shortpath)
...
def is_battle_start_button_visible():
    ...
    img_gray = ADB.screencap_mat(gray=True, std_size=True)
    pos_rect = res_nav.get_pos("/before_battle/start_button")
    cropped = imgops.mat_crop(img_gray, Rect(*pos_rect))
    if not os.path.exists(res_img.get_img_path(template_shortpath)):
        cv2.imwrite(res_img.get_img_path(template_shortpath), cropped)
    terminal_template = res_img.get_img_gray(template_shortpath)
...
def expand_menu_bar():
    ...
    pos_rect = res_nav.get_pos(pos_shortpath)
...
def menu_bar_press_main_menu():
    pos_shortpath = "/navigate_bar/nav_main"
    pos_rect = imgops.from_std_rect(ADB.get_resolution(), Rect(*res_nav.get_pos(pos_shortpath)))
    ADB.input_press_rect(imgops.from_std_rect(ADB.get_resolution(), Rect(*pos_rect)))
    time.sleep(1)
...
def handle_unknown_scene():
    if battle.is_finished():
        ...
        press_std_rect("/navigate_bar/back_black")
    # TODO: feat. 自动上线
...
def handle_close_button():
    ...
    img_template = res_img.get_img_bgr(template_shortpath)
    img_mask = res_img.get_img_bgr(mask_shortpath)
...
def is_public_recruit():
    template_shortpath = "/public_recruit/flag.png"
    img_gray = ADB.screencap_mat(gray=True, std_size=True)
    pos_rect = res_nav.get_pos("/public_recruit/flag")
    cropped = imgops.mat_crop(img_gray, Rect(*pos_rect))
    if not os.path.exists(res_img.get_img_path(template_shortpath)):
        cv2.imwrite(res_img.get_img_path(template_shortpath), cropped)
    terminal_template = res_img.get_img_gray(template_shortpath)
...
def main_menu_press_public_recruit():
    press_std_rect("/main_menu/public_recruit")


def menu_bar_press_public_recruit():
    press_std_rect("/navigate_bar/nav_public_recruit")


def nav_to_public_recruit():
    error_counter = 0
    while not is_public_recruit():
        if is_main_menu():
            main_menu_press_public_recruit()
        elif expand_menu_bar():
            menu_bar_press_public_recruit()
            continue
        elif handle_back_button():
            continue
        elif handle_reward_scene():
...
def nav_resource_choose_category(stage: str):
    ...
    img_template = res_img.get_img_gray(template_path)
...
def nav_get_stagemap_and_choose_stage_ocr(stage: str, error_tolerance=10):
    ...
    stage_map = res_nav.get_stage_map_local(stage)
    if stage_map is None:
        stage_info = res_nav.get_stage_info_and_map(stage)
...
def record_new(path: str, start_from_terminal=True):
    ...
    records = []
    record_data = {
        'hw_ratio': res_img.get_img_path_suffix(),
